...
async fn main() -> Result<()> {
    ...
    match args.command {
        Commands::Analyze(analyze_args) => {
            ...
            let model_paths = match llm::download_model(&model_repo, &model_file).await {
                Ok(paths) => paths,
                Err(e) => {
            ...
            let device = llm::select_device();
            let mut pipeline = llm::LlmPipeline::new(model_paths, device)?;
            ...
            let prompt = llm::build_prompt(&input_text, final_prompt_template);

            println!("{}", "LogTrains: Analyzing input...".cyan().bold());
            println!("\n{}", "=== Explanation ===".green().bold());

            let res = pipeline.run(&prompt, |token| {
                print!("{}", token);
                io::stdout().flush()?;
                Ok(())
            });
